...
def evaluate_testing_metrics(pta, testing_pos_traces, testing_neg_traces, verbose=True):
        """
        用 testing 正例 / 负例计算 TP, FN, TN, FP 与 BCR。
        - 正例：应被接受
        - 负例：应被拒绝
        """
        if testing_pos_traces is None:
            testing_pos_traces = []
        if testing_neg_traces is None:
            testing_neg_traces = []
        tp = fn = 0
        for tr in testing_pos_traces:
            ok, _ = pta.accepts_trace(tr)
            if ok:
                tp += 1
            else:
                fn += 1
        fp = tn = 0
        for tr in testing_neg_traces:
            ok, _ = pta.accepts_trace(tr)
            if ok:
                fp += 1
            else:
        ...
        if verbose:
            print("\n================ Testing Metrics (TP/FN/TN/FP & BCR) ================")
            print(f"TP (正例且接受): {tp}")
            print(f"FN (正例却拒绝): {fn}")
            print(f"TN (负例且拒绝): {tn}")
            print(f"FP (负例却接受): {fp}")
            print(f"TPR (正例召回): {tpr:.6f}")
            print(f"TNR (负例特异度): {tnr:.6f}")
            print(f"BCR = (TPR+TNR)/2: {bcr:.6f}")
            print(f"Accuracy = (TP+TN)/all: {acc:.6f}")
            print("=====================================================================\n")
        return {
            "TP": tp,
            "FN": fn,
...
class PTA:
    ...
    def merge_rejects_all_negatives(self, state_1, state_2, negative_traces):
        """
        在副本上尝试 merge(state_1 <- state_2)。
        若合并后仍有任一条 negative 被接受，返回 False（不允许合并）。
        negative_traces 为空时视为无约束，返回 True。
        """
        if not negative_traces:
            return True
        if state_1 is None or state_2 is None or state_1.id == state_2.id:
            return True
        sim = copy.deepcopy(self)
        id_to_state = {s.id: s for s in sim.states_list}
        if state_1.id not in id_to_state or state_2.id not in id_to_state:
            return False
        sim.merge_states(id_to_state[state_1.id], id_to_state[state_2.id])
        for tr in negative_traces:
            ok, _ = sim.accepts_trace(tr)
            if ok:
                return False
        return True

    def k_tail_merge(self, k=None, negative_traces=None):
        """
        稳定版 k-tail：每次只合并一对；合并前若会导致某条 negative 被接受则跳过该对。
        """
        ...
        while True:
            ...
            for i in range(len(current_states)):
                ...
                for j in range(i + 1, len(current_states)):
                    ...
                    if t1 != t2:
                        continue
                    if not self.merge_rejects_all_negatives(s1, s2, negative_traces):
                        rejected_by_negative += 1
                        print(
                            f"⛔ 跳过合并 S{s1.id} <- S{s2.id}（会接受某条 negative），"
                            f"累计跳过: {rejected_by_negative}"
                        )
                        continue
                    merged_pair = (s1, s2)
                    break
            ...
            step += 1
        print(f"\n📌 因 negative 约束跳过的合并次数: {rejected_by_negative}")
        print("\n🎉 k-tail 合并完成")
        self.print_pta()
...
def main():
    training_pos_traces = read_trace_file(file_path="D:/NonEntertainment/Life_Data/UK_Sheffield/Module/COM6911_Team_Project/adaptive-k-tails-automata/data/automaton_5s/automaton_5s/training_data/11_pos.txt")
    training_neg_traces = read_trace_file(file_path="D:/NonEntertainment/Life_Data/UK_Sheffield/Module/COM6911_Team_Project/adaptive-k-tails-automata/data/automaton_5s/automaton_5s/training_data/11_neg.txt")
    ...
    pta.k_tail_merge(k=2, negative_traces=training_neg_traces)
    
    testing_pos_traces = read_trace_file(file_path="D:/NonEntertainment/Life_Data/UK_Sheffield/Module/COM6911_Team_Project/adaptive-k-tails-automata/data/automaton_5s/automaton_5s/testing_data/11_pos.txt")
    testing_neg_traces = read_trace_file(file_path="D:/NonEntertainment/Life_Data/UK_Sheffield/Module/COM6911_Team_Project/adaptive-k-tails-automata/data/automaton_5s/automaton_5s/testing_data/11_neg.txt")
    ...
    metrics = evaluate_testing_metrics(
    pta,
    testing_pos_traces,
    testing_neg_traces,
    verbose=True,
