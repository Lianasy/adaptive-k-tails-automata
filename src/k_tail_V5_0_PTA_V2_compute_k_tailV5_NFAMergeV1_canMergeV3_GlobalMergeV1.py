...
class Automaton():
    ...
    # ========================
        def check_all_nfa_conflicts(s1: State, s2: State, visited: set) -> bool:
            ...
            if pair in visited:
                print(f"Circle meet✅ S{s1.id} vs S{s2.id} NFA checked")
                return True
            visited.add(pair)
            ...
            print(f"keep-S{s1.id} = {trans1}")
            print(f"delete-S{s2.id} = {trans2}")

            # 遍历 s2 的所有边，检查是否与 s1 冲突
            for sym, t2 in s2.transitions:
                if sym not in trans1:
                    continue
                print(f"S{s1.id} vs S{s2.id} find conflict sym = {sym}")

                # 冲突：同一个符号 → 两个目标
                t1 = trans1[sym]

                print(f"   Check NFA conflict: [{s1.id},{s2.id}] sym='{sym}' → S{t1.id} vs S{t2.id}")

                # 规则1：accept 不同 → 直接非法
                if t1.is_accept != t2.is_accept:
                    print(f"❌ Conflict: tgt accept different → S{t1.id}={t1.is_accept}, S{t2.id}={t2.is_accept} -- > lead to check_all_nfa_conflict() return False")
                    return False

                # 规则2：accept 相同 → 必须递归检查这两个子节点
                if t1.is_accept == t2.is_accept:
                    print(f"✅ tgt accept same → S{t1.id}={t1.is_accept}, S{t2.id}={t2.is_accept}")
                    print(f"chekcing child NFA conflict validation S{t1.id} vs S{t2.id}")
                    child_NFA_check = check_all_nfa_conflicts(t1, t2, visited)
                    if child_NFA_check == False:
                        print(f"❌ Recursive NFA check Fail for S{t1.id} & S{t2.id} -- > lead to check_all_nfa_conflict() return False")
                        return False
                    if child_NFA_check == True:
                        print(f"✅ Recursive NFA check PASS for S{t1.id} & S{t2.id} -- > next round of FOR loop of sym check of S{s1.id} vs S{s2.id}")

            # 所有冲突都检查完毕 ✅
            print(f"✅ S{s1.id} vs S{s2.id} all conflict checked and PASS")
            return True
        ...
        for state in self.states_Dict.values():
            ...
            for sym, tgt in state.transitions:
                if tgt.id == delete_state_id:
                    # 原来指向被删除状态 → 重定向到保留状态
                    unique_trans[sym] = keep_state
                else:
                    # 不变
                    unique_trans[sym] = tgt
            # 字典转回列表 → 完全无重复！
            state.transitions = [(s, t) for s, t in unique_trans.items()]

    def NFA_merge(self, state_id: int) -> None:
        """
        【纯递归消除 NFA】
        规则：
        ...
        if state_id not in self.states_Dict:
            print(f"S({state_id} no exists, can't do NFA merge)")
            return False
        ...
        for sym, target_states in sym_groups.items():
            if len(target_states) <= 1:
                print(f"✅ S{state_id} [{sym}] has no repeted target")
                continue  # 没有NFA，跳过
            if len(target_states) > 1:
    ...
    def Global_merge(self, k: int) -> None:
        """
        全局自动合并算法（严格按你的要求实现）
        1. 遍历所有状态两两配对
        2. 先检查 k-tail 是否相等
        ...
        while if_merged:
            ...
            for i in range(state_total):
                ...
                for j in range(i + 1, state_total):
                    ...
                    kt_state1 = self.compute_state_k_tail(state1_id, k)
                    kt_state2 = self.compute_state_k_tail(state2_id, k)
                    ...
                    if kt_state1 == kt_state2:
                        ...
                        canMerge = self.can_merge(state1_id, state2_id)
