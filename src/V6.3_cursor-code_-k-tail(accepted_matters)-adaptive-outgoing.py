...
class PTA:
    ...
    def get_pair_adaptive_k_by_outgoing(self, s1, s2, k_vector, mode="min", leaf_fallback_k=1):
        """
        根据两个状态的共同出边字符决定k：
        - 正常：共同出边 -> 取min/max对应k
        - 特例：两个状态都为叶子（无出边）-> 回退到 leaf_fallback_k
        - 其他 common_out 为空情况 -> 不比较
        """
        out1 = {sym for sym, _ in s1.transitions}
        out2 = {sym for sym, _ in s2.transitions}
        common_out = out1 & out2

        # 新增：两者都是叶子，允许比较
        if not out1 and not out2:
            return leaf_fallback_k, set()

        # 原逻辑：无共同出边则不比较
        if not common_out:
            return None, set()

        k_values = [k_vector[sym] for sym in common_out if sym in k_vector]
        if not k_values:
            return None, common_out

        if mode == "max":
            return max(k_values), common_out
        return min(k_values), common_out

    def adaptive_k_tail_merge(self, k_vector, mode="min"):
        """
        稳定版 Adaptive k-tail 合并（按共同出边字符选k）：
        - 每次只做一次合并
        - 合并后立刻在最新图上重新搜索下一对
        """
        step = 1
        while True:
            ...
            print(f"\n===== 第{step}次 adaptive k-tail 合并搜索 (mode={mode}) =====")
            # 在“当前最新图”中找第一对可合并状态
            for i in range(len(current_states)):
                ...
                for j in range(i + 1, len(current_states)):
                    ...
                    if s1.id == s2.id:
                        continue
                    # 先用共同出边字符决定本次k
                    k_pair, common_out = self.get_pair_adaptive_k_by_outgoing(
                                            s1, s2, k_vector, mode=mode, leaf_fallback_k=1
                                            )
                    if k_pair is None:
                        continue
                    t1 = self._k_tail(s1, k_pair)
                    t2 = self._k_tail(s2, k_pair)
                    if t1 == t2:
                        merged_pair = (s1, s2, k_pair, common_out)
                        break
            ...
            s1, s2, k_pair, common_out = merged_pair
            print(f"✅ 合并 S{s1.id} <- S{s2.id} | common_out={common_out} | k={k_pair}")
            self.merge_states(s1, s2)
            print(f"当前状态数: {len(self.states_list)}")
            step += 1
        print("\n🎉 adaptive k-tail 合并完成")
        self.print_pta()


def main():
    training_pos_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/1_pos.txt")
    ...
    pta.adaptive_k_tail_merge(k_vector=k_vector, mode="min")
    
    training_pos_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/7_neg.txt")
