...
class PTA:
    ...
    def adaptive_k_tail_merge_strict(self, k_vector, using_maximum_or_minimum_k="min"):
        """
        严格模式：仅当两个状态的进入字符集完全相同时，才尝试合并（仅用极值k判断）
        :param k_vector: 字典形式，如{'L0':1, 'L1':1}
        :param using_maximum_or_minimum_k: 极值选择，可选"max"/"min"；默认使用"min"
        """
        # 1. 找到每个状态的父状态和触发字符
        parent_map = self.find_state_parents()
        # 2. 遍历所有状态对（避免重复，只比较i<j）
        merged = set()  # 记录已合并的状态ID
        states_copy = self.states_list.copy()  # 复制状态列表，避免合并后遍历异常
        
        for i in range(len(states_copy)):
            s1 = states_copy[i]
            # 跳过已合并/已删除的状态
            if s1.id in merged or s1 not in self.states_list:
                continue
                    
            for j in range(i+1, len(states_copy)):
                s2 = states_copy[j]
                if s2.id in merged or s2 not in self.states_list:
                    continue

                # 3. 严格模式核心：判断进入字符集是否完全相同
                s1_triggers = set([p[1] for p in parent_map.get(s1.id, [])])
                s2_triggers = set([p[1] for p in parent_map.get(s2.id, [])])
                
                # 字符集不完全相同 → 直接跳过
                if s1_triggers != s2_triggers:
                    continue
                
                # 字符集完全相同 → 提取共同字符集
                common_triggers = s1_triggers
                if not common_triggers:
                    continue  # 无进入字符（初始状态），跳过
                ...
                if using_maximum_or_minimum_k == "max":
                    ...
                    # 参数无效时默认用最小值
                    print(f"❌ 无效的极值参数：{using_maximum_or_minimum_k}，默认使用最小值")
                    k = min(common_k_values)
                    k_type = "最小值"
                
                print(f"\n🔍 严格模式：验证状态{s1.id}和{s2.id}（共同字符集{k_type}k值：{k}）")
                print(f"################ Searching {k} tail for s1 #################")
                s1_tail = self.get_k_tail(s1, k)
                print(f"################ Done Searching {k} tail for s1 #################")
                print(f"################ Searching {k} tail for s2 #################")
                s2_tail = self.get_k_tail(s2, k)
                print(f"################ Done Searching {k} tail for s2 #################")
                
                if s1_tail == s2_tail:
                    print(f"✅ 严格模式：合并状态S{s1.id}和S{s2.id}（共同字符集{k_type}k值：{k}）")
                    self.merge_states(s1, s2)
                    merged.add(s2.id)
                    print(f"⚠️  状态S{s2.id}已标记为已合并，后续将跳过\n")
...
def main():
    training_pos_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/2_pos.txt")
    ...
    pta.get_k_tail(state=pta.initial_state,k=3)
    pta.find_state_parents()
    # pta.merge_states(state_left=pta.states_list[1],state_right=pta.states_list[4])
    # pta.print_pta()
    pta.adaptive_k_tail_merge_not_strict(k_vector=k_vector, using_maximum_or_minimum_k=max)
