...
class Automaton():
    ...
    def k_common_outgoing(
        ...
        if k_list:
            ...
            k_decided = max(k_list) if aggregate == "max" else min(k_list)
    ...
    def Global_merge(self, verbose:bool = False) -> None:
        """
        全局自动合并算法（严格按你的要求实现）
        1. 遍历所有状态两两配对
        2. 先检查 k-tail 是否相等
        ...
        while if_merged:
            ...
            for i in range(state_total):
                ...
                for j in range(i + 1, state_total):
                    ...
                    k = self.k_common_outgoing(state1_id,state2_id,aggregate="max")
...
def single_infer(
    ...
    atm.Global_merge(verbose=global_merge_verbose)
