...
def distance_aware_loss(logits, targets, num_classes, distance_weight=0.2):
    """
    Cross-entropy + distance-aware penalty for ordinal k-value prediction.

    logits:  [batch_size, num_symbols, num_classes]
    targets: [batch_size, num_symbols], zero-based class indices

    CrossEntropyLoss only checks whether the predicted class is correct.
    The distance-aware term additionally penalizes probability mass that is
    far away from the true k class, which is useful because k-values are ordinal.
    """
    ...
    class_ids = torch.arange(
        ...
        dtype=torch.float32
    ).view(1, 1, num_classes)

    target_ids = targets.float().unsqueeze(-1)
    abs_distance = torch.abs(class_ids - target_ids)

    expected_distance = (probs * abs_distance).sum(dim=-1).mean()
    total_loss = ce_loss + distance_weight * expected_distance

    return total_loss, ce_loss, expected_distance
...
'''def train_multitask_model(model, X_train, y_train, X_val, y_val, epochs=200, lr=0.0005):
    ...
    for epoch in range(epochs):
        ...
        # and [B, S] into [B*S].
        train_loss, train_ce_loss, train_distance_loss = distance_aware_loss(
            pred,
            y_train,
            num_classes,
        ...
        with torch.no_grad():
            ...
            val_loss, val_ce_loss, val_distance_loss = distance_aware_loss(
                pred_val,
                y_val,
                num_classes,
                distance_weight=distance_weight
    ...
    return model'''
def train_multitask_model(model, X_train, y_train, X_val, y_val, epochs=300, lr=0.0005, distance_weight=0.2):
    import copy
    ...
    for epoch in range(epochs):
        ...
        if epoch % 20 == 0:
            print(
                ...
                f"Train Loss: {train_loss.item():.4f} | "
                f"Val Loss: {val_loss.item():.4f} | "
                f"Train CE: {train_ce_loss.item():.4f} | "
                f"Train DistLoss: {train_distance_loss.item():.4f} | "
                f"Val DistLoss: {val_distance_loss.item():.4f} | "
...
def runMultiTaskMLP(folder, num_k_classes=4, epochs=200, lr=0.0005, distance_weight=0.2):
