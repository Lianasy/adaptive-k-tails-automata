...
class PTA:
    ...
    def _k_tail(self, state, k, visited=None):
        """
        计算状态的k-tail签名，用于比较是否可合并。
        - k=0时只保留当前状态是否accept的信息
        - 递归展开后续转移（按symbol排序，保证可比较）
        """
        if visited is None:
            visited = set()
        # 防止合并后出现环导致递归异常
        ...
        for sym, nxt in state.transitions:
            sub = self._k_tail(nxt, k - 1, visited)
...
def main():
    training_pos_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/7_pos.txt")
    ...
    pta.k_tail_merge(k=2)
    
    training_pos_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/6_pos.txt")
