...
class PTA:
    ...
    def merge_is_safe_with_negatives(self, s1_id, s2_id, negative_traces):
        """
        在副本上尝试合并 s1 <- s2。
        如果导致任一负例被接受，则返回False（不安全）。
        """
        pta_copy = self.clone()
        # 在副本里按id找对应状态对象
        id_to_state = {s.id: s for s in pta_copy.states_list}
        if s1_id not in id_to_state or s2_id not in id_to_state:
            return False
        pta_copy.merge_states(id_to_state[s1_id], id_to_state[s2_id])

        # 检查负例：只要负例中有一条被接受，就拒绝此合并
        for tr in negative_traces:
            ok, _ = pta_copy.accepts_trace(tr)
            if ok:
                return False
        return True
    ...
    def k_tail_merge(self, k=2, negative_traces=None):
        ...
        rejected_by_negative = 0   # 新增：被负例约束拒绝的合并次数
        accepted_merges = 0        # 可选：真正执行的合并次数

        while changed:
            changed = False
            states_snapshot = self.states_list.copy()

            print(f"\n===== 第{round_idx}轮 k-tail 合并 (k={k}) =====")
            for i in range(len(states_snapshot)):
                s1 = states_snapshot[i]
                if not self._state_exists(s1):
                    continue

                for j in range(i + 1, len(states_snapshot)):
                    s2 = states_snapshot[j]
                    ...
                    safe = self.merge_is_safe_with_negatives(s1.id, s2.id, negative_traces)
                    if not safe:
                        rejected_by_negative += 1   # 新增计数
                        print(f"❌ 拒绝合并 S{s1.id} <- S{s2.id}（会接收负例）")
                        continue

                    print(f"✅ 合并 S{s1.id} <- S{s2.id}（通过负例约束）")
                    self.merge_states(s1, s2)
                    accepted_merges += 1            # 可选计数
                    changed = True

            print(f"第{round_idx}轮结束，当前状态数: {len(self.states_list)}")
            round_idx += 1

        print("\n🎉 k-tail 合并完成")
        print(f"📌 因负例约束被拒绝的合并次数: {rejected_by_negative}")   # 你要的打印
        print(f"📌 实际执行的合并次数: {accepted_merges}")                # 可选
        self.print_pta()

def main():
    training_pos_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/8_pos.txt")
    training_neg_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/8_neg.txt")
    ...
    training_pos_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/8_neg.txt")
