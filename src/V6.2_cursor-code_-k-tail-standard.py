...
class PTA:
    ...
    def _k_signature(self, state, k, memo):
        """
        标准k-tail签名：
        - 包含accept信息
        - 递归展开到深度k
        ...
        if key in memo:
            return memo[key]
        if k == 0:
            sig = ("ACC", state.is_accept)
            memo[key] = sig
            return sig
        items = []
        for sym, nxt in state.transitions:
            sub = self._k_signature(nxt, k - 1, memo)
            items.append((sym, sub))
        items.sort(key=lambda x: (x[0], str(x[1])))
        sig = ("ACC", state.is_accept, tuple(items))
        memo[key] = sig
        return sig
    ...
    def k_tail_merge_standard(self, k=2):
        """
        标准 DFA-fold 风格：
        - 先按 k-signature 分组
        - 再做确定化闭包（同组同符号多目标 -> 合并目标组）
        - 最后一次性构造新DFA
        """
        # ---------- 1) 初始分组：按k-tail签名 ----------
        memo = {}
        ...
        for st in self.states_list:
            sig = self._k_signature(st, k, memo)
            sig_to_group.setdefault(sig, []).append(st.id)

        # old_state_id -> class_id
        old_to_class = {}
        class_members = []  # list[set(old_id)]
        for cid, members in enumerate(sig_to_group.values()):
            mset = set(members)
            class_members.append(mset)
            for oid in mset:
                old_to_class[oid] = cid

        old_id_map = {s.id: s for s in self.states_list}

        # ---------- 2) 并查集工具 ----------
        parent = list(range(len(class_members)))
        ...
        # ---------- 3) 确定化闭包：解决同symbol多目标 ----------
        changed = True
        while changed:
            changed = False

            # root -> all old states in this merged class
            root_members = {}
            for cid, members in enumerate(class_members):
                r = find(cid)
                root_members.setdefault(r, set()).update(members)

            for r, members in list(root_members.items()):
                # symbol -> set(target_root)
                sym_targets = {}
                for oid in members:
                    st = old_id_map[oid]
                    for sym, tgt in st.transitions:
                        tgt_c = old_to_class[tgt.id]
                        tgt_r = find(tgt_c)
                        sym_targets.setdefault(sym, set()).add(tgt_r)

                # 同一symbol多个目标类 -> 目标类两两合并
                for sym, tset in sym_targets.items():
                    if len(tset) <= 1:
                        continue
                    tlist = sorted(tset)
                    base = tlist[0]
            ...
            if changed:
                # 每次并类后，更新 old_to_class 映射到 root
                for oid, cid in list(old_to_class.items()):
                    old_to_class[oid] = find(cid)
        ...
        final_roots = sorted(root_members.keys())
        root_to_new = {}
        new_states = []
        for new_id, r in enumerate(final_roots):
            ns = State(state_id=new_id)
            root_to_new[r] = ns
            new_states.append(ns)

        # old_id -> new_state
        old_to_new = {}
        for oid, cid in old_to_class.items():
            old_to_new[oid] = root_to_new[find(cid)]

        # accept: 组内OR
        for r, members in root_members.items():
            ns = root_to_new[r]
            ns.is_accept = any(old_id_map[oid].is_accept for oid in members)

        # transitions: 组内所有成员出边汇总，且每symbol唯一目标
        for r, members in root_members.items():
            ns = root_to_new[r]
            sym_to_target = {}
            for oid in members:
                st = old_id_map[oid]
                for sym, tgt in st.transitions:
                    nt = old_to_new[tgt.id]
                    if sym in sym_to_target and sym_to_target[sym] is not nt:
                        # 理论上闭包后不应冲突；保护性处理
                        if nt.id < sym_to_target[sym].id:
                            sym_to_target[sym] = nt
                    else:
                        sym_to_target[sym] = nt
            ns.transitions = [(sym, sym_to_target[sym]) for sym in sorted(sym_to_target.keys())]

        # 初态映射
        self.initial_state = old_to_new[self.initial_state.id]
        self.states_list = new_states

        print(f"\n🎉 标准 k-tail DFA-fold 合并完成 (k={k})")
        self.print_pta()

def main():
    training_pos_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/3_pos.txt")
    ...
    pta.k_tail_merge_standard(k=4)
    
    training_pos_traces = read_trace_file(file_path="./Input/data/data/5 state automata/1/training_data/3_pos.txt")
    pta.validate_traces(training_pos_traces, verbose=True)
