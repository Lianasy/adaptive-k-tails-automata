...
def test_do_single_merge(
    ...
    before_count = len(atm_case_3.states_Dict)

    result = atm_case_3.do_single_merge(keep_state_id, delete_state_id)

    assert result is True
    assert keep_state_id in atm_case_3.states_Dict
    assert delete_state_id not in atm_case_3.states_Dict
    assert len(atm_case_3.states_Dict) == before_count - 1

    keep_state = atm_case_3.states_Dict[keep_state_id]
    ...
    for source_id, label, new_target_id in redirected_edges:
        assert (label, new_target_id) in atm_case_3.states_Dict[source_id].get_transition()
        
    for state in atm_case_3.states_Dict.values():
